...
from TwFA.modules.maskgit.vqgan_vae import VQGanVAE
from TwFA.modules.maskgit.t5 import t5_encode_text, get_encoded_dim, DEFAULT_T5_NAME
...
class Transformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.pos_emb = nn.Embedding(seq_len + 150, dim)
    ...
    def forward(
        ...
        cond_drop_prob = 0.,
        conditioning_token_ids: Optional[torch.Tensor] = None, # [bs, 150]
        ...
        if exists(conditioning_token_ids):
            ...
            # import ipdb; ipdb.set_trace()
            x = torch.cat([conditioning_token_ids, x], dim=-1)
            total_len = x.shape[-1]
            # cond_token_emb = self.token_emb(conditioning_token_ids)
            # context = torch.cat((context, cond_token_emb), dim = -2) # here
            # context_mask = F.pad(context_mask, (0, conditioning_token_ids.shape[-1]), value = True)
            x = self.token_emb(x)
            x = x + self.pos_emb(torch.arange(total_len, device = device))
        ...
        if exists(conditioning_token_ids):
            # cut off the conditional tokens
            logits = logits[: , -n: ]
...
class MaskGit(nn.Module):
    ...
    def generate(
        self,
        texts: List[str] = None,
        text_embd: Optional[torch.Tensor] = None,
        ...
        # add support for cond_embed and cond_ids
        if self.vae != None and fmap_size == None:
        ...
        if texts:
            ...
            text_embeds = self.transformer.encode_text(texts) # [bs, nax_len, embed_dim]
        elif text_embd:
            # assert text_embd != None
            batch_size = text_embd.shape[0]
            text_embeds = text_embd
            del text_embd
        else:
            assert cond_token_ids  != None
            batch_size = cond_token_ids.shape[0]
            text_embeds = None

        # import ipdb; ipdb.set_trace()
        shape = (batch_size, seq_len)

        ids = torch.full(shape, self.mask_id, dtype = torch.long, device = device) # [4, 256] full of masked ids
        ...
        for timestep, steps_until_x0 in tqdm(zip(torch.linspace(0, 1, timesteps, device = device), reversed(range(timesteps))), total = timesteps):
            ...
            num_token_masked = max(int((rand_mask_prob * seq_len).item()), 1) # 1--> 256
        ...
        if not exists(self.vae) or return_ids:
            return ids
    ...
    def forward(
        ...
        rand_time = uniform((batch,), device = device) # 随机数
        rand_mask_probs = self.noise_schedule(rand_time) #
        num_token_masked = (seq_len * rand_mask_probs).round().clamp(min = 1) # masked掉的数目

        mask_id = self.mask_id
        batch_randperm = torch.rand((batch, seq_len), device = device).argsort(dim = -1)
        mask = batch_randperm < rearrange(num_token_masked, 'b -> b 1') # 得到mask

        mask_id = self.transformer.mask_id # -1
        labels = torch.where(mask, ids, ignore_index) # 把mask=False对应的ids变成-1

        if self.no_mask_token_prob > 0.: # skip
            no_mask_mask = get_mask_subset_prob(mask, self.no_mask_token_prob)
            mask &= ~no_mask_mask

        x = torch.where(mask, mask_id, ids) # 把mask=True的地方ids变成8192

        # get text embeddings

        if exists(texts):
            text_embeds = self.transformer.encode_text(texts) # [bs, max_len, embd_dim]
            texts = None
